...
public class BotDecisionMakeTask extends BotTaskAutoParams<BotDecisionMakeTaskParams> {

    public BotDecisionMakeTask(Bot bot) {
        super(bot, null, BotDecisionMakeTaskParams.class);
    }

    @Override
    public IBotTaskParameterized<BotDecisionMakeTaskParams> setParams(BotDecisionMakeTaskParams params) {
     ...
     * if (!BotInventory.hasFreeInventorySpace(bot, BotMaterialUtils.dirtTypes) ||
     * BotInventory.hasEnoughBlocks(bot, BotMaterialUtils.dirtTypes, maxToCollect))
     * {
     ...
     * 
     * BotLogger.info(this.isLogged(),"📦 " + bot.getId() + " Идёт к точке сброса: "
     * + BotStringUtils.formatLocation(drop_off_loc));
     * return;
     * }
     * 
     ...
     * boolean isNight = (time >= 13000 && time <= 23000); // Примерно 13000 -
     * закат, 23000 - рассвет
     * 
     * double huntChance = isNight ? 0.5 : 0.1; // 90% ночью, 10% днем
     * 
     * if (rand < 0.1) { // 10% шанс сказать что-то про окружающий мир
     * BotLogger.info(this.isLogged(),"🤖 " + bot.getId() +
     * " Комментирует обстановку.");
     * bot.addTaskToQueue(new BotTalkTask(bot, null,
     * BotTalkTask.TalkType.ENVIRONMENT_COMMENT));
     * return;
     * }
     * 
     * 
     * if (rand < huntChance) {
     * // ⚔️ Охота
     * BotLogger.info(this.isLogged(),"⚔️ " + bot.getId() +
     * " Собирается на охоту! (Вероятность: " + huntChance * 100 + "%)");
     * 
     * BotHuntMobsTask hunt_task = new BotHuntMobsTask(bot);
     * //BotTaskHuntMobs config = hunt_task.getConfig();
     * Set<EntityType> a_targets = ((BotHuntTaskConfig)
     * hunt_task.getConfig()).getTargetAggressiveMobs();
     * Set<EntityType> p_targets = ((BotHuntTaskConfig)
     * hunt_task.getConfig()).getTargetPassiveMobs();
     * Set<EntityType> targets = isNight ? a_targets : p_targets;
     * 
     * hunt_task.configure(targets, 20, true);
     * bot.addTaskToQueue(hunt_task);
     * BotLogger.info(this.isLogged(), "⚔️ " + bot.getId() + " Начинает охоту на " +
     * (isNight ? "агрессивных мобов" : "животных") + "!");
     * 
     * return;
     * }
     * 
     ...
     * breakTask.configure(dirtTypes, maxToCollect, BotConstants.DEFAULT_SCAN_RANGE,
     * true);
