...
public class BotNavigationUtils {
    ...
    public static void navigateTo(Bot bot, BotLocation target, float multiplier) {
        if (target == null) {
            BotLogger.debug(bot.getActiveTask().getIcon(), true,
                    bot.getId() + " ⛔ Target navigation is null. Can't navigate!");
            return;
        }
    ...
    private static void navigate(Bot bot, BotLocation target, float multiplier) {
        ...
        BotLogger.debug(bot.getActiveTask().getIcon(), true, bot.getId() + " 🏃🏻‍♂️‍➡️ Wants to navigate to "
                + target.toString() + " [ID: " + bot.getBrain().getCurrentTask().getIcon() + "]");
        BotLogger.debug(bot.getActiveTask().getIcon(), true,
                bot.getId() + " 🎯 Runtime Target Location: " + bot.getNavigation().getTarget().toString() + " [ID: "
                        + bot.getBrain().getCurrentTask().getIcon() + "]");
