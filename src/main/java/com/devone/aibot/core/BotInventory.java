...
public class BotInventory {
    ...
    public void pickupAll(Boolean shouldPickup, Boolean autoPickupEnabled) {
        ...
        if (!shouldPickup || !autoPickupEnabled || !bot.isNPCSpawned() || bot.getNPC() == null) {
            BotLogger.debug(true, "🛒 " + bot.getId() + " Не будет подбирать материал! Параметры подбора: " + shouldPickup
                    + " | " + autoPickupEnabled);
            return;
        }

        BotLogger.debug(true, "🛒 " + bot.getId() + " Будет подобирать материал! Параметры подбора: " + shouldPickup + " | "
                + autoPickupEnabled);
    ...
    public static boolean equipRequiredTool(Bot bot, Material blockType) {
        ...
        for (int i = 0; i < inv.getSize(); i++) {
            ...
            if (item != null && item.getType() == requiredTool) {
                ...
                BotLogger.trace(true, "🤖 Взял в руку инструмент: " + requiredTool);
                return true;
            }
        }

        BotLogger.trace(true, "🧰 Инструмент " + requiredTool + " не найден в инвентаре");
        return false;
    }
