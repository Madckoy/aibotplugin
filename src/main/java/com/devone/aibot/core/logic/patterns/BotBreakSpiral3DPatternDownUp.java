...
public class BotBreakSpiral3DPatternDownUp implements IBotBreakPattern {
    ...
    public Location findNextBlock(Bot bot, Map<Location, ?> geoMap, Set<Material> targetMaterials) {
        ...
        List<Location> candidates = geoMap.keySet().stream()
            ...
            .filter(loc -> {
                ...
                // ❗ Игнорируем только строго центральную вертикаль (X=0, Z=0)
                return !(dx == 0 && dz == 0);
            })
    ...
    public String getName() {
        return "BotBreakSpiralFullClearPattern";
    }
}
